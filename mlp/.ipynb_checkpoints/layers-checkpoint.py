...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        """
        if stochastic:
            # Determine the mask's shape based on whether it's shared across the batch.
            mask_shape = (1,) + inputs.shape[1:] if self.share_across_batch else inputs.shape
            # Create a random array based on the mask shape.
            random_values = self.rng.rand(*mask_shape)
            # Apply the inclusion probability threshold to create the mask.
            self._mask = np.where(random_values < self.incl_prob, 1, 0)
            # Apply the mask to the inputs (element-wise multiplication).
            outputs = inputs * self._mask
        else:
            # In the deterministic scenario, scale the outputs by the inclusion probability.
