...
class L1Penalty(object):
    ...
    def __call__(self, parameter):
        ...
        return np.multiply(self.coefficient, np.sum(np.abs(parameter)))
...
class L2Penalty(object):
    ...
    def __call__(self, parameter):
        ...
        Returns:
            Value of penalty term.
        """
        squared_param = np.power(parameter, 2)
        sum_of_squares = np.sum(squared_param)
        return 0.5 * self.coefficient * sum_of_squares
